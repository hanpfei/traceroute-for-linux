...
static const char *addr2str(const sockaddr_any *addr) {
	getnameinfo(&addr->sa, sizeof(*addr), addr2str_buf, sizeof(addr2str_buf), 0,
			0, NI_NUMERICHOST);
...
int main(int argc, char *argv[]) {
	...
	if (wait_secs < 0 || here_factor < 0 || near_factor < 0)
		ex_error("bad wait specifications `%g,%g,%g' used", wait_secs,
				here_factor, near_factor);
	...
	if (af == AF_INET6 && (tos || flow_label))
		dst_addr.sin6.sin6_flowinfo = htonl(
...
static void print_header(void) {
	...
	printf("traceroute to %s (%s), %u hops max, %zu byte packets", dst_name,
			addr2str(&dst_addr), max_hops, header_len + data_len);
	fflush(stdout);
}
...
void tune_socket(int sk) {
	...
	if (af == AF_INET) {
		...
		if (setsockopt(sk, SOL_IP, IP_MTU_DISCOVER, &i, sizeof(i)) < 0
				&& (!dontfrag
						|| (i = IP_PMTUDISC_DO, setsockopt(sk, SOL_IP,
								IP_MTU_DISCOVER, &i, sizeof(i)) < 0)))
		...
		if (setsockopt(sk, SOL_IPV6, IPV6_MTU_DISCOVER, &i, sizeof(i)) < 0
				&& (!dontfrag
						|| (i = IPV6_PMTUDISC_DO, setsockopt(sk, SOL_IPV6,
								IPV6_MTU_DISCOVER, &i, sizeof(i)) < 0)))
		...
		if (flow_label) {
			...
			if (setsockopt(sk, IPPROTO_IPV6, IPV6_FLOWLABEL_MGR, &flr,
					sizeof(flr)) < 0)
		...
		if (tos || flow_label) {
			i = 1;
			if (setsockopt(sk, IPPROTO_IPV6, IPV6_FLOWINFO_SEND, &i, sizeof(i))
					< 0)
...
void recv_reply(int sk, int err, check_reply_t check_reply) {
	...
	if (ee && mtudisc && ee->ee_info >= header_len
	...
	header_len <= 128
			...
			((af == AF_INET
					...
					|| (af == AF_INET6
							&& (ee->ee_type == ICMP6_TIME_EXCEEDED
...
void bind_socket(int sk) {
	...
	if (device) {
		if (setsockopt(sk, SOL_SOCKET, SO_BINDTODEVICE, device,
				strlen(device) + 1) < 0)
